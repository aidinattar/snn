...
class InceptionSNN(NetworkTrainer):
    ...
    def punish(self, branch_idx):
        """Punish the network"""
        if branch_idx == 0:
            self.block3_1['anti_stdp'](self.ctx["input_spikes"], self.ctx["potentials"], self.ctx["output_spikes"], self.ctx["winners"])
        elif branch_idx == 1:
            self.block3_2['anti_stdp'](self.ctx["input_spikes"], self.ctx["potentials"], self.ctx["output_spikes"], self.ctx["winners"])
        elif branch_idx == 2:
            self.block3_3['anti_stdp'](self.ctx["input_spikes"], self.ctx["potentials"], self.ctx["output_spikes"], self.ctx["winners"])
        else:
            raise ValueError("Invalid branch index")
