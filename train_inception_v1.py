...
from inceptionSNN_v1 import inception_v1  # Import your Inception V1 model here
from inceptionSNN_v1 import S1C1Transform
from inceptionSNN_v1 import train_unsupervise
from inceptionSNN_v1 import train_rl
from inceptionSNN_v1 import test
...
if __name__ == "__main__":
    ...
    if os.path.isfile(f"models/inceptionSNN_v1_{args.dataset}_first_layer.pth"):
        print("Loading first layer")
        snn.load_state_dict(
            torch.load(f"models/inceptionSNN_v1_{args.dataset}_first_layer.pth"),
            strict = False
        )
    else:
        for epoch in range(args.epochs_1):
            ...
            if epoch % args.save_every == 0:
                if not os.path.isdir("models"):
                    os.mkdir("models")
                torch.save(snn.state_dict(), f"models/inceptionSNN_v1_{args.dataset}_first_layer.pth")
        # save model
        if not os.path.isdir("models"):
            os.mkdir("models")
        torch.save(snn.state_dict(), f"models/inceptionSNN_v1_{args.dataset}_first_layer.pth")
    ...
    if os.path.isfile(f"models/inceptionSNN_v1_{args.dataset}_second_layer.pth"):
        print("Loading second layer")
        snn.load_state_dict(
            torch.load(f"models/inceptionSNN_v1_{args.dataset}_second_layer.pth"),
            strict = False
        )
    else:
        for branch in range(3):
            if os.path.isfile(f"models/inceptionSNN_v1_{args.dataset}_second_layer_branch{branch}.pth"):
                print(f"Loading branch {branch}")
                snn.load_state_dict(
                    torch.load(f"models/inceptionSNN_v1_{args.dataset}_second_layer_branch{branch}.pth"),
                    strict = False
                )
            else:
                ...
                for epoch in range(args.epochs_2):
                    ...
                    if epoch % args.save_every == 0:
                        if not os.path.isdir("models"):
                            os.mkdir("models")
                        torch.save(snn.state_dict(), f"models/inceptionSNN_v1_{args.dataset}_second_layer_branch{branch}.pth")

        # save model
        if not os.path.isdir("models"):
            os.mkdir("models")
        torch.save(snn.state_dict(), f"models/inceptionSNN_v1_{args.dataset}_second_layer.pth")
        ...
        for epoch in range(args.epochs_3):
            ...
            for data, targets in tqdm(test_loader, desc="Testing"):
                ...
                if best_test[0] <= performance_test[0]:
                    ...
                    if not os.path.isdir("models"):
                        os.mkdir("models")
                    torch.save(snn.state_dict(), f"models/inceptionSNN_v1_best_model.pth")
    ...
    if not os.path.isdir("models"):
        os.mkdir("models")
    torch.save(snn.state_dict(), f"models/inceptionSNN_v1_{args.dataset}_third_layer.pth")
