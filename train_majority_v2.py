...
from majoritySNN_v2 import majoritySNN
from majoritySNN_v2 import S1C1Transform
from majoritySNN_v2 import train_unsupervised
from majoritySNN_v2 import train_rl
from majoritySNN_v2 import test
...
if __name__ == "__main__":
    ...
    if os.path.isfile(f"models/majoritySNN_{args.dataset}_first_layer.pth"):
        snn.load_state_dict(
            torch.load(f"models/majoritySNN_{args.dataset}_first_layer.pth"),
            strict=False
        )
    else:
        for epoch in range(args.epochs_1):
            ...
            if epoch % args.save_every == 0:
                if not os.path.isdir("models"):
                    os.mkdir("models")
                torch.save(snn.state_dict(), f"models/majoritySNN_{args.dataset}_first_layer.pth")
        # Save model
        if not os.path.isdir("models"):
            os.mkdir("models")
        torch.save(snn.state_dict(), f"models/majoritySNN_{args.dataset}_first_layer.pth")
    ...
    if os.path.isfile(f"models/majoritySNN_{args.dataset}_second_layer.pth"):
        snn.load_state_dict(
            torch.load(f"models/majoritySNN_{args.dataset}_second_layer.pth"),
            strict=False
        )
    else:
        for epoch in range(args.epochs_2):
            ...
            if epoch % args.save_every == 0:
                if not os.path.isdir("models"):
                    os.mkdir("models")
                torch.save(snn.state_dict(), f"models/majoritySNN_{args.dataset}_second_layer.pth")
    ...
    apr = snn.stdp3_branch1.learning_rate[0][0].item()
    anr = snn.stdp3_branch1.learning_rate[0][1].item()
    app = snn.anti_stdp3_branch1.learning_rate[0][1].item()
    anp = snn.anti_stdp3_branch1.learning_rate[0][0].item()
        ...
        for epoch in range(args.epochs_3):
            ...
            for data, targets in test_loader:
                ...
                if best_test[0] <= performance_test[0]:
                    ...
                    if not os.path.isdir("models"):
                        os.mkdir("models")
                    torch.save(snn.state_dict(), f"models/majoritySNN_best_model.pth")
    ...
    if not os.path.isdir("models"):
        os.mkdir("models")
    torch.save(snn.state_dict(), f"models/majoritySNN_{args.dataset}_third_layer.pth")
